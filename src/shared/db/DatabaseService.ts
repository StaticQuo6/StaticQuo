...
export async function initVaultDb(): Promise<SQLiteDBConnection> {
  const conn = getSqlite()

  const ret = await conn.checkConnectionsConsistency()
  const isConn = await conn.isConnection('staticquo-vault.db', false)

  if (ret.result && isConn.result) {
    vaultDb = await conn.retrieveConnection('staticquo-vault.db', false)
  } else {
    vaultDb = await conn.createConnection(
      'staticquo-vault.db',
      false,
      'no-encryption',
      1,
  ...
  const tableExists = await vaultDb.isTable('vault_meta')
...
export async function initSearchDb(): Promise<SQLiteDBConnection> {
  ...
  const ret = await conn.checkConnectionsConsistency()
  const isConn = await conn.isConnection('staticquo-search.db', false)

  if (ret.result && isConn.result) {
    searchDb = await conn.retrieveConnection('staticquo-search.db', false)
  } else {
    searchDb = await conn.createConnection(
      'staticquo-search.db',
      false,
      'no-encryption',
      1,
  ...
  const tableExists = await searchDb.isTable('protocols')
  if (!tableExists.result) {
...
export async function openMbtilesDb(
  ...
  return conn.createConnection(dbPath, false, 'no-encryption', 1, true)
}
